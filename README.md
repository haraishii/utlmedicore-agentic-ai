...
2. 🕸️ **Hybrid Graph-Memory Engine**: Menyimpan struktur data interaktif dengan **Neo4j + Graphiti AI**.
...
3. ☁️ **Cloud & Local LLM Ready**: Integrasi mulus menggunakan **Ollama** antara pemrosesan LLM lokal (`llama3.1:8b`) bebas biaya, atau pengalihan ke Cloud API (`mistral-large-3:675b` / `kimi-k2-thinking`) untuk penalaran (reasoning) yang berat.
...
## 🏗️ Arsitektur Sistem & Cara Kerjanya

1. **Frontend (Bento Dashboard)**
   Berbasis HTML/CSS/JS (Flask Templates). Menyajikan formulir untuk menambahkan *Manual Context* (Makanan/Peristiwa), serta tombol pemicu (*trigger*) yang mengaktifkan pembuatan laporan AI (Report Generation).
...
4. **Agent Layer (CrewAI / Lite Agent)**
   Saat user meminta laporan, *Agent* akan menarik seluruh *memory/graph* dari Neo4j. AI (Llama3.1) lalu mengevaluasi data, mendeteksi anomali, dan menulis laporan medis.
...
└── 📁 utils /                       # Modul general (misal: tz_utils.py untuk sinkronisasi waktu)
```
...
Sistem ini bisa dijalankan langsung di _local environment_ (Conda) atau menggunakan **Docker**.
...
pip install -r requirements.txt  # Jika tersedia, atau install Flask, Neo4j, CrewAI secara manual
```

**3. Konfigurasi Lingkungan (`.env`)**
Buat pastikan keberadaan file `.env` di *root* dan sesuaikan dengan contoh environment ini:
```env
...
```
Aplikasi akan aktif di `http://127.0.0.1:5000` atau `http://localhost:5000` (sesuai konfigurasi Flask).
...
---
**Maintained by:** Tim UTLMediCore | Agentic AI Integration
